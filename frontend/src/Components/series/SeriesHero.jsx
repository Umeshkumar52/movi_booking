...
import { Edit, Trash2, ArrowLeft, Star, Calendar, Layers, PlayCircle, Globe, Activity } from 'lucide-react';
...
export default function SeriesHero({ series, setSeriesData }) {
  ...
  async function postDeleteHandler() {
    if (!window.confirm("Are you sure you want to delete this series and all associated seasons/episodes?")) return;
    try {
      ...
      <div className="relative w-full h-[600px] bg-slate-950 text-white overflow-hidden shadow-2xl">
        ...
        <div className="absolute inset-0">
          <img
            src={data?.poster}
            alt="backdrop"
            className="w-full h-full object-cover opacity-30 blur-md transform scale-110"
          />
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto h-full flex flex-col md:flex-row items-center gap-12 px-8 py-12">
          ...
          <div className="flex-1 flex flex-col justify-center text-center md:text-left">
            <div className="flex items-center gap-3 mb-4 justify-center md:justify-start">
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-[0.2em] backdrop-blur-md border ${data.isCompleted ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-10 justify-center md:justify-start">
              <div className="flex items-center gap-2 px-4 py-2 bg-slate-900/50 rounded-2xl border border-white/5 backdrop-blur-md transition-all hover:bg-slate-900 hover:border-indigo-500/30">
                ...
                <span className="text-sm font-black text-slate-200">IMDb {data?.rating || "N/A"}</span>
              </div>
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-slate-900/50 rounded-2xl border border-white/5 backdrop-blur-md">
                ...
                <span className="text-sm font-bold text-slate-300">{data?.year}</span>
              </div>

              <div className="flex items-center gap-2 px-4 py-2 bg-slate-900/50 rounded-2xl border border-white/5 backdrop-blur-md">
                <Layers size={16} className="text-purple-400" />
                <span className="text-sm font-bold text-slate-300">{data?.totalSeasons} Seasons</span>
              </div>

              <div className="flex items-center gap-2 px-4 py-2 bg-slate-900/50 rounded-2xl border border-white/5 backdrop-blur-md">
                <PlayCircle size={16} className="text-blue-400" />
                <span className="text-sm font-bold text-slate-300">{data?.totalEpisodes} Episodes</span>
              </div>

              <div className="flex items-center gap-2 px-4 py-2 bg-slate-900/50 rounded-2xl border border-white/5 backdrop-blur-md">
                <Globe size={16} className="text-emerald-400" />
                <span className="text-sm font-bold text-slate-300 tracking-wide uppercase">{data?.language}</span>
              </div>
            </div>

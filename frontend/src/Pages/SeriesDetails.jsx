...
import { Layout, Users, Star, Tv, Info, Plus } from "lucide-react";
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-6">
                <div>
                  <h3 className="text-sm font-black uppercase tracking-[0.2em] text-indigo-400 mb-2">Storyline</h3>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-[0.2em] text-indigo-400 mb-2">Genres</h3>
                    <div className="flex flex-wrap gap-2">
                      {seriesData.genres?.split(',').map(genre => (
                        <span key={genre} className="px-3 py-1 bg-slate-800 border border-white/5 rounded-lg text-xs font-bold text-slate-400">
                          {genre.trim()}
                        </span>
                      ))}
                    </div>
                  ...
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-[0.2em] text-indigo-400 mb-2">Release Date</h3>
                    <p className="text-slate-300 font-bold">{new Date(seriesData.releaseDate).toLocaleDateString()}</p>
                  </div>
                </div>
              </div>
              <div className="bg-slate-800/30 rounded-[2rem] border border-white/5 p-6 h-fit space-y-6">
                <h3 className="text-sm font-black uppercase tracking-[0.2em] text-indigo-400">Quick Stats</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500 text-xs font-bold uppercase tracking-widest">Pricing</span>
                    <span className={`text-sm font-black ${seriesData.premium ? 'text-amber-400' : 'text-emerald-400'}`}>
                      {seriesData.premium ? `$${seriesData.price}` : "FREE"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500 text-xs font-bold uppercase tracking-widest">Views</span>
                    <span className="text-sm font-black text-slate-200">{seriesData.views || 0} Total</span>
                  </div>
                   <div className="flex items-center justify-between">
                    <span className="text-slate-500 text-xs font-bold uppercase tracking-widest">Language</span>
                    <span className="text-sm font-black text-slate-200 uppercase">{seriesData.language}</span>
                  </div>
                </div>
              </div>
            </div>
